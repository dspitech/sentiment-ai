pipeline {
    ...
    stages {
            ...
            steps { 
                ...
                withSonarQubeEnv('sonarqube') {
                    withCredentials([string(credentialsId: 'sonar-token', variable: 'SONAR_TOKEN')]) {
                        // On utilise l'option -u 0:0 pour s'assurer que les fichiers appartiennent à root 
                        // et on ajuste les permissions immédiatement après pour que Jenkins puisse lire le rapport
                        sh """
                        docker run --rm -u 0:0 -v ${WORKSPACE}:/usr/src -w /usr/src sonarsource/sonar-scanner-cli:latest sonar-scanner \
        ...
        stage('7. Security Scan') { steps { sh "docker run --rm -v /var/run/docker.sock:/var/run/docker.sock -v trivy-cache:/root/.cache/trivy aquasec/trivy:latest image --severity HIGH,CRITICAL --ignore-unfixed --exit-code 1 ${IMAGE_NAME}:${IMAGE_TAG}" } }
        stage('8. Push') { when { branch 'main' } steps { withCredentials([usernamePassword(credentialsId: 'github-token', usernameVariable: 'REG_USER', passwordVariable: 'REG_PASS')]) { sh "echo \$REG_PASS | docker login ghcr.io -u \$REG_USER --password-stdin && docker tag ${IMAGE_NAME}:${IMAGE_TAG} ${REGISTRY}/${IMAGE_NAME}:${IMAGE_TAG} && docker push ${REGISTRY}/${IMAGE_NAME}:${IMAGE_TAG}" } } }
